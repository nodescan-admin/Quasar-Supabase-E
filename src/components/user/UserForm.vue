...
'Записать' 'Write (Update)
 'Имя' 'Name'
'Телефон' 'Telephone'
'Иван Иванов' 'Ivan Ivanov'
Адрес электронной почты E-mail address
Введите отображаемое имя Enter a name to be Displayed?
Отображаемое имя Displayed name?
Введите номер мобильного телефона Enter your mobile ph number
Номер мобильного телефона Telephone number

Removed from Telephone fld
   // prefix='+7'
  ...
  .pr-form.q-mt-xl
    q-form.q-px-lg.column.q-gutter-y-none(ref='userform' @submit='handleSubmit' style='width:40rem;')
      ...
      q-input(
        ...
        mask='(###) ###-##-##'
        lazy-rules
        :rules='[ val => !!val || "Enter your mobile ph number"]'
        )
          template(v-slot:prepend)
            q-icon(name='fas fa-phone')
          q-tooltip Telephone number
